...
-- EMPLOYEE테이블에서 사원이름(3글자) 중 가운데 글자가 '하'인 사원들의 사원명, 전화번호 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '_하_';  -- 무조건 3글자
-- WHERE EMP_NAME LIKE '_하%';  -- 이름이 외자인 사원도 가능  김하, 김하보, 이하인하
-- WHERE EMP_NAME LIKE '%하_';  -- 하보, 김하보, 선우하보
...
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';   -- KIM_JI@NAVER.COM, lee_@~, idk_dki@~

-- 와일드 카드로 사용되고 있는 문자와 컬럼값에 들어 있는 문자가 동일하기 때문에 조회 안됨
-- 모두다 와일드카드로 인식
...
SELECT EMP_ID, SALARY, HIRE_DATE, BONUS
FROM EMPLOYEE
...
-- 전 사원의 연봉(보너스포함)별 내림차순 정렬 이때 NULL값은 맨 마지막에 나오도록 조회(이름, 연봉)
-- SELECT EMP_NAME, (SALARY+SALARY*BONUS)*12 "보너스포함 연봉"
SELECT EMP_NAME, (SALARY*(1+BONUS))*12 "보너스포함 연봉"
FROM EMPLOYEE
ORDER BY (SALARY+(SALARY*BONUS))*12 DESC NULLS LAST;